...
This implementation operates in O(n) time due to an interesting quirk of it's
use case.

Because the implementation is intended to be operated with one word against
many words, we can use a levenshtein automaton[0] to evaluate an entire
dictionary in O(|m|) time by constructing a non-deterministic finite-state
automaton (NFA), where the states of the machine represent the costs as defined
by the original levenshtein distance algorithm.

The implementation itself is not my work, rather the work of Arachnid[1]. I
understand the NFA and DFA implementation, however I would rather somebody else
(with more experience) implement the data structures and algorithms. No license
or attribution is mentioned by the author, however I will credit his work and
blog post[2] description the implementation.

[0]: https://en.wikipedia.org/wiki/Levenshtein_automaton
[1]: https://gist.github.com/Arachnid/491973
